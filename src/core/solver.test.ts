...
import { tileGrid, tileDigit, tileTime, tileTimeGrid, TIME_ROWS, TIME_COLS } from "./solver";
...
describe("tileDigit", () => {
  ...
  it("should produce correct number of pieces for each digit", () => {
    ...
    for (let d = 0; d <= 9; d++) {
      ...
      expect(result.pieces.length, `Digit ${d} should have ${expectedPieces} pieces`).toBe(expectedPieces);
    }
  });
...
describe("tileTimeGrid", () => {
  it("should tile time 12:34 into one unified grid", () => {
    const result = tileTimeGrid(12, 34, { seed: 42 });

    expect(result.success).toBe(true);
    expect(result.grid.length).toBe(TIME_ROWS);
    expect(result.grid[0]?.length ?? 0).toBe(TIME_COLS);

    // Total cells should be divisible by 4 (tetromino tiling)
    expect((TIME_ROWS * TIME_COLS) % 4).toBe(0);
    expect(result.pieces.length * 4).toBe(TIME_ROWS * TIME_COLS);
  });

  it("should be reproducible with same seed", () => {
    const r1 = tileTimeGrid(23, 59, { seed: "test-seed" });
    const r2 = tileTimeGrid(23, 59, { seed: "test-seed" });

    expect(r1.success && r2.success).toBe(true);
    expect(r1.pieces.map((p) => p.id)).toEqual(r2.pieces.map((p) => p.id));
    expect(r1.pieces.map((p) => p.type)).toEqual(r2.pieces.map((p) => p.type));
  });

  it("should throw for invalid hours/minutes", () => {
    expect(() => tileTimeGrid(-1, 0)).toThrow();
    expect(() => tileTimeGrid(24, 0)).toThrow();
    expect(() => tileTimeGrid(12.5, 0)).toThrow();
    expect(() => tileTimeGrid(0, -1)).toThrow();
